...
proc swapp_get_name {} {
    return "HR FreeRTOS Hello World";
}
...
proc swapp_is_supported_hw {} {
    ...
    if { $proc_type == "microblaze"} {
        ...
        if { [llength $timerlist] <= 0 } {
            ...
            if { [llength $timerlist] <= 0 } {
                error "There seems to be no timer peripheral in the hardware.";
            }
        }
    }
	...
	if { $proc_type == "psu_pmu"} {
		error "ERROR: not supported on psu_pmu";
		return;
	}
...
proc swapp_is_supported_sw {} {
	...
	if {$processor_type == "psu_cortexa53"} {
		...
		if {[string compare -nocase $compiler "arm-none-eabi-gcc"] == 0} {
			error "ERROR: library does not support 32 bit A53 compiler";
		return;
            }
	}
